...
document.addEventListener('visibilitychange', () => {
  ...
  if (document.visibilityState === 'visible' && currentDepositKey) {
    const commandRef = firebase.database().ref(`deposits/${currentDepositKey}/command`);
    ...
    commandRef.once('value')
      .then((snapshot) => {
        ...
        if (currentCommand === 'PAGAR') {
          ...
          setTimeout(() => {
            commandRef.set('CONSULTAR_COBRANÇA')
              .then(() => {
                console.log('Comando CONSULTAR_COBRANÇA enviado');
                hideProgressDialog();
              })
              .catch((error) => {
                console.error('Erro ao enviar comando CONSULTAR_COBRANÇA: ', error);
              });
...
function handleParticipation(sweepstakeKey, ticketPrice, path) {
    if (path === 'sweepstakes3') {
        const code = prompt("Por favor, insira o código para participar do sorteio.");
        ...
        const participateButton = document.querySelector(`button[onclick="handleParticipation('${sweepstakeKey}', ${ticketPrice}, '${path}')"]`);
...
function listenForCallback(key, participateButton) {
    ...
    callbackRef.on('value', (snapshot) => {
        ...
        if (callbackData && callbackData.type === "sweepstakes" && callbackData.text) {
            hideProgressDialog();
            showToast(callbackData.text);
            participateButton.disabled = false;
            callbackRef.remove();
        }
        if (callbackData && callbackData.type === "create-draw" && callbackData.text) {
            hideProgressDialog();
            showToast(callbackData.text);
            callbackRef.remove();
        }
...
function showIndicate() {
    ...
    if (user) {
        ...
        if (navigator.share) {
            ...
            alert('O compartilhamento não é suportado pelo seu navegador. Copie o link: ' + indicateLink);
        }
    } else {
...
function registrarVisita() {
            const dataAtual = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Sao_Paulo' }).replace(/-/g, '');
...
function displayUserInfo(user) {
    ...
    if (profile) {
            ...
            if (element) {
                if (selector === 'img') {
                    ...
                    element.addEventListener('click', () => {
                        const newPhotoURL = prompt('Insira o URL da nova imagem de perfil:');
                        if (newPhotoURL) {
                            user.updateProfile({ photoURL: newPhotoURL })
                                .then(() => {
                                    element.src = newPhotoURL;
                                    // Atualizar a imagem no Firebase Database
                                    return firebase.database().ref(`users/${user.uid}/profile`).update({ image: newPhotoURL });
                                })
                                .then(() => {
                                    console.log('Foto de perfil atualizada com sucesso no Auth e no Database!');
                                })
                                .catch(error => {
                        ...
                        if (newName) {
                            user.updateProfile({ displayName: newName })
                                .then(() => {
                                    ...
                                    return firebase.database().ref(`users/${user.uid}/profile`).update({ name: newName });
                                })
                                .then(() => {
                                    console.log('Nome atualizado com sucesso no Auth e no Database!');
                                })
                                .catch(error => {
        ...
        const navbarBalanceElement = document.querySelector('.navbar-title .balancee');
        ...
        const referredPointsRef = firebase.database().ref(`users/${user.uid}/referred_points`);
        referredPointsRef.on('value', snapshot => {
...
function handleLinkClick(path) {
            ...
            links.forEach(link => {
                ...
                if (linkPath === path) {
                    ...
                    link.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' }); // Centraliza o link na viewport
                } else {
...
function updateOrientationText(path) {
            ...
            switch (path) {
                case 'sweepstakes0':
                    orientationText.textContent = 'Participe dos sorteios gratuitos e aguarde completar a quantidade de participantes para que o sorteio ocorra.';
                    break;
                case 'sweepstakes':
                    orientationText.textContent = 'Participe dos sorteios pagos para ter a chance de ganhar prêmios maiores.';
                    break;
                case 'sweepstakes4':
                    orientationText.innerHTML = 'Indique amigos e ganhe pontos para participar dos sorteios por indicação. <a href="#" onclick="showIndicate()">Compartilhar</a>';
                    break;

                case 'sweepstakes5':
                    orientationText.innerHTML = 'Aqui voce encontra os sorteios criado pelos usuarios. <a href="#" onclick="createSweepstake()">Clique aqui</a> e crie um novo sorteio.';
                    break;
                    
                case 'sweepstakes6':
                    orientationText.innerHTML = 'De centavo em centavo, seu prêmio é garantido';
                    break;
    ...
    break;
                ...
                case 'sweepstakes2':
                    ...
                    break;
                default:
                    orientationText.textContent = 'Selecione um tipo de sorteio para mais informações.';
                    break;
            }
}
...
const participateButton = (isParticipating && (ticketPrice === 0 || path === 'sweepstakes4'))
    ...
    let ticketValueText = ticketPrice === 0 ? 'Ingresso: <span class="ticket-red">Gratuito</span>' : `Ingresso: <span class="ticket-red">${formatCurrency(sweepstake.ticket)}</span>`;
    if (path === 'sweepstakes3') {
        ticketValueText = '';
    }
    if (path === 'sweepstakes4') {
        ticketValueText = `Ingresso: <span class="ticket-red">${ticketPrice} ponto${ticketPrice > 1 ? 's' : ''}</span>`;
    }
    ...
    const creatorText = sweepstake.description ? `<p class="creator-text">${sweepstake.description}</p>` : '';
    
    const sweepstakeHash = hashCode(sweepstakeKey);
    
    const sweepstakeItem = `
    <div class="sweepstake-item ${itemClass} ${path === 'sweepstakes2' ? 'sweepstake-green' : ''}">
        ...
        <div class="sweepstake-others">
            ...
            ${path !== 'sweepstakes2' ? `<button class="btn-view" onclick="viewParticipants('${oldKey || sweepstakeKey}', '${winnerKey}')">Visualizar</button>` : ''}
        </div>
    </div>`;
...
function recharge() {
    ...
    if (user) {
        ...
        firebase.database().ref(`deposits/${uid}`).update(depositData)
            .then(() => {
            ...
            .catch(error => {
                ...
                alert('Erro ao enviar a solicitação de recarga. Por favor, tente novamente.');
            });
    } else {
        ...
        if (user) {
            ...
            db.ref(`withdrawal_requests`).push(requestData)
                ...
                .catch(error => {
                    ...
                    showToast('Erro ao enviar a solicitação de resgate. Por favor, tente novamente.');
                    closeModal2();
                });
        } else {
            alert('Usuário não autenticado.');
...
function validatePixKey(pixType, pixKey) {
    ...
    switch (pixType) {
        ...
        case 'PHONE':
            ...
            errorMessage = 'Por favor, insira um número de celular válido com 11 dígitos.';
            break;
        case 'EMAIL':
            regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        ...
        case 'EVP':
            ...
            errorMessage = 'Por favor, insira uma chave aleatória válida com 32 caracteres.';
            break;
        default:
            return false;
    }
...
function cancelPix() {
      ...
      const commandRef = firebase.database().ref(`deposits/${uid}/command`);
        commandRef.set('DELETAR_COBRANÇA')
          .then(() => {
...
function updateNotificationCount(notifications) {
    ...
    if (notifications) {
        let localStorageNotifications = JSON.parse(localStorage.getItem('list_notifications'));
        ...
        const ids_notifications_array = ids_notifications ? ids_notifications.split(',') : [];

        Object.values(notifications).forEach((notification) => {
            if (uid && (!ids_notifications || !ids_notifications_array.includes(notification.key.toString())) && (notification.to === uid || notification.to === 'all') && !isNotificationRead(notification.key)) {
...
function openNotificationsPopup() {
    ...
    if (popup) {
        ...
        const notifications = JSON.parse(localStorage.getItem('list_notifications')) || {};
...
document.getElementById('prizeValue').addEventListener('input', updateInvestmentInfo);
document.getElementById('numParticipants').addEventListener('input', updateInvestmentInfo);
...
function updateInvestmentInfo() {
    ...
    if (!isNaN(prizeValue) && prizeValue >= 2 && !isNaN(numParticipants) && numParticipants >= 5) {
        ...
        const potentialEarningsText = `Você ganhará: R$${(prizeValue + (prizeValue / 4)).toFixed(2)}`;

        document.getElementById('investmentText').innerText = investmentText;
        document.getElementById('potentialEarningsText').innerText = potentialEarningsText;
    } else {
...
document.getElementById('createButton').addEventListener('click', function() {
    ...
    if (!isNaN(prizeValue) && prizeValue >= 2) {
      if (!isNaN(numParticipants) && numParticipants >= 5) {
        ...
        // Envie os dados atualizados para o Firebase
        firebase.database().ref(`create_draw/${keyy}`).set(send)
            .then(() => {
                // Sucesso na gravação
...
initialMessage.textContent = 'Estamos melhorando o site todos os dias para oferecer a melhor experiência em sorteios do Brasil';
...
function showIndiqueDialog() {
            const rankingRef = db.ref('ranking_ref');
            rankingRef.orderByChild('f').limitToLast(10).once('value', snapshot => {
                const rankingList = document.getElementById('ranking-list');
                rankingList.innerHTML = ''; // Limpa a lista antes de popular
                let rank = 1;

                snapshot.forEach(childSnapshot => {
                    const userq = childSnapshot.val();
                    const listItem = document.createElement('li');
                    listItem.innerHTML = `
                        <span class="ranking-position">${rank++}.</span>
                        <img src="${userq.i}" alt="${userq.n}" class="user-image">
                        <span class="user-name">${userq.n}</span>
                        <span class="friends-count">${userq.f} amigos indicados</span>
                    `;
                    rankingList.prepend(listItem); // Adiciona em ordem decrescente
                });

                // Substitua os valores pelos valores das variáveis referred_friends e referred_points
                document.getElementById('total-friends').innerText = referred_friends;
                document.getElementById('total-points').innerText = referred_points;

                document.getElementById('indique-dialog').style.display = 'block';
            });
        }
        ...
        document.getElementById('use-points-button').addEventListener('click', function() {
            window.location.href = 'https://bloi308.com/sorteios?tab=sweepstakes4';
            document.getElementById('indique-dialog').style.display = 'none';
        });
